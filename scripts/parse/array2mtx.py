...
def writeMatrics(dic, outname, samples, positions, regiondic, refdic, varcount):
	...
	outfile.write('POS\tREF\tALT\tGENE\tAF\t')
...
#		print ref, len(filter(lambda x: x != ref+ref, varlist))
		...
		chrom = position.split(':')[0]; pos = position.split(':')[1]
#		if 'rs' in positionid:
#			positionid = 'rs' + positionid.split('rs')[1].split('_')[0].split('.')[0]

		line += chrom + ':' + pos +'\t'+ref+'\t'
#		outfile.write(position+'\t'+ref+'\t')
		dp = len(list(''.join(samcalls.values())))
		alts = filter(lambda x: x != ref, list(''.join(samcalls.values())))
#		print ref, alts, dp, len(alts), float(len(alts))/dp
		af = float(len(alts))/dp
